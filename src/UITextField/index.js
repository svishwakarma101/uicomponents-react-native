...
export default function UITextField(props) {
  ...
  function getTextFieldStyle() {
    return StyleSheet.flatten([
      ...
      errorMessage&&{marginBottom:(errorMessage.length/35)*10>10?(errorMessage.length/35)*10:10}
    ]);
  }

  function getAccessoryView() {
    if (
      props.rightAccessoryView &&
      typeof props.rightAccessoryView === "function"
    ) {
  ...
  function getRightSideClearButton() {
    ...
    if (showClearButton || isClearButton) {
      let clearButtonSource = props.clearButtonImage
        ? props.clearButtonImage
        : cancelIcon;
          ...
          {isClearButton ? (
            ...
            <Image
              style={[
                ...
                {
                  tintColor:
                    showPlaceHolderForFloating || textFieldNotEmpty
  ...
  function getRightSideShowButton() {
    ...
    if (props.showShowHideButton || (isShowButton && isHideButton)) {
      ...
      return (
        <TouchableOpacity
          style={[styles.rightAccessory, props.rightAccessoryStyle]}
          onPress={showHideSecureText}
        >
          {isShowButton && isHideButton ? (
            shouldEncryptedTextBeVisible ? (
              hideButton()
            ) : (
              showButton()
  ...
  function getLeftAccessoryView() {
    if (
      props.leftAccessoryView &&
      typeof props.leftAccessoryView === "function"
    ) {
  ...
  function clearText() {
    if (props.showClearButton !== 1 && !props.disabledClear) {
      // If the entered text is valid then avoid the text clear action.
      clear();
      if (props.onChangeText) {
        props.onChangeText("");
      }
  ...
  function getLabelAccessory() {
    if (
      props.renderLabelRightAccessory &&
      typeof props.renderLabelRightAccessory === "function"
    ) {
      ...
      <View style={styles.underlineContainerView}>
        <TextField
          ...
          titleTextStyle={[
            ...
            errorMessage ? errorMessageStyle : descriptionStyle,
          ]}
          ...
          labelColor={
            !disabled
              ? labelColor || theme.TextField.textColor
              : placeholderColor || theme.TextField.placeholderTextColor
          }
          baseColor={placeholderColor || theme.TextField.placeholderTextColor}
          ...
          placeholderTextColor={
            !isFloating ? placeholderColor || "" : placeholderColor || null
          }
          ...
          maxLength={props.maxLength || getMaxLength(props.type)}
          accessibilityLabel={
            accessibilityLabel
              ? accessibilityLabel
              : accessibilityId(
                  testID
                    ...
                    : getAccessibilityLabel(
                        floatingLabel || props.name || "textfield"
                      ),
                  floatingLabel || props.name
                )
          }
          testID={
            testID
